...
new_skin_selector = """function SkinSelector({ selectedSkin, onSkinChange }) {
...
# Nájdi SkinSelector a nahraď
start = next(i for i, l in enumerate(lines) if 'function SkinSelector(' in l)
...
new_lines = lines[:start] + new_skin_selector.splitlines() + lines[end:]
open('src/App.jsx', 'w', encoding='utf-8').write('\n'.join(new_lines) + '\n')
print(f'Done. Replaced lines {start+1}-{end}. Total: {len(new_lines)}')
