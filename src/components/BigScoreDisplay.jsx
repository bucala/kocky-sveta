...
// Layout sa nehýbe, menia sa iba hodnoty.
export function BigScoreDisplay({ players, totals, highlightPlayer, target, extensions = {}, size = 'lg' }) {
    ...
    >
      {players.map((p, i) => {
        ...
        return (
          <div
            key={i}
            className={`ks-card rounded-sm ${cardPad} flex flex-col items-center justify-center text-center gap-2 transition-colors ${
          ...
          >
            <div className="flex items-center gap-2 min-w-0 max-w-full">
              ...
              {extensions.coloredAvatars && (
                <span
                  ...
                  style={{
                    width: crownSize, height: crownSize, fontSize: crownSize * 0.42,
                    background: PLAYER_COLORS[i % PLAYER_COLORS.length], color: '#1a1410',
                  }}
                >
                  {getInitials(p)}
                </span>
              ...
              <span
                className={`ks-display font-semibold truncate ${isCurrent ? 'ks-gold' : 'ks-cream'}`}
            ...
            <div
              className={`ks-display font-bold leading-none ${t < 0 ? 'text-red-300' : reached ? 'ks-gold' : 'ks-cream'}`}
              style={{
                fontSize: scoreSize,
                color: extensions.coloredAvatars && t >= 0 ? PLAYER_COLORS[i % PLAYER_COLORS.length] : undefined,
              }}
            >
